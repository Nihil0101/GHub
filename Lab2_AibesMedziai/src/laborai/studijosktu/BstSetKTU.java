...
public class BstSetKTU<E extends Comparable<E>> implements SortedSetADT<E>, Cloneable {
    ...
    //man reikia realizuoti //maybe private
    ...
    public SortedSetADT<E> subSet(E element1, E element2) {
        SortedSetADT<E> poaibis = new BstSetKTU<>();
        for (E e : this) {
     ...
     *
     * @param element1 - pradinis aibės poaibio elementas.
     * @param fi - true arba false pradžios
     * @param element2 - galinis aibės poaibio elementas.
     * @param ti - true arba false galo
     * @return Grąžinamas aibės poaibis nuo elemento element1 iki element2.
     */
    //@Override mano rašytas
    public SortedSetADT<E> subSet(E element1, boolean fi, E element2, boolean ti) {
        SortedSetADT<E> poaibis = new BstSetKTU<>();
        for (E e : this) {
            if (e.compareTo(element1) >= 0 && fi == true && e.compareTo(element2) <= 0 && ti == true) {
                poaibis.add(e);
            }
        }
            return poaibis;
    ...
    public SetADT<E> tailSet(E element) {
        SetADT<E> poaibis = new BstSetKTU<>(); //naujsa medis
        for (E e : this) { //nuo elemento iki šito elemnot
            if (e.compareTo(element) >= 0) { //jeigu daugiau
                poaibis.add(e); //dedam į naują meddį
            }
        }
        return poaibis; //gražinam naują medį
    }

    //arba užkomentuotas arba ne
    /*
    /**
     * Grąžinamas aibės poaibis iki elemento.
     *
     ...
     * @return Grąžinamas aibės poaibis nuo elemento.
     */
    public SortedSetADT<E> tailSet1(E element) {
        SortedSetADT<E> poaibis = new BstSetKTU<>(); //naujsa medis
        for (E e : this) { //nuo elemento iki šito elemnot
            if (e.compareTo(element) >= 0) { //jeigu daugiau
                poaibis.add(e); //dedam į naują meddį
            }
    ...
    private class IteratorKTU implements Iterator<E> {
        ...
        public void remove() {
            ...
            //BstSetKTU.this.remove(n.element); jei neveik pirmasis
        }
