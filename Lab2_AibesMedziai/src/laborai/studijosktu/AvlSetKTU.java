...
public class AvlSetKTU<E extends Comparable<E>> extends BstSetKTU<E>
    ...
    private AVLNode<E> removeRecursive(E element, AVLNode<E> node) {
        ...
        int cmp = c.compare(element, node.element); //čia bus -1, 0 arba 1

        if (cmp < 0) { // jeigu 1
            node.setLeft(removeRecursive(element, node.getLeft())); //pašalinam kairę dalį

            if ((height(node.getLeft()) - height(node.getRight())) == 2) { //jeigu kairio ir dešinio skirtumas yra 2
                int cmp2 = c.compare(element, node.getLeft().element); //čia bus -1, 0 arba 1

                node = (cmp2 < 0) ? rightRotation(node) : doubleRightRotation(node); // išlyginam
            }
        } else if (cmp > 0) { //jeigu -1
            node.setRight(removeRecursive(element, node.getRight())); //pašalinam dešinę dalį

            if ((height(node.getRight()) - height(node.getLeft())) == 2) { //jeigu kairio ir dešinio skirtumas yra 2
                int cmp2 = c.compare(node.getRight().element, element); //čia bus -1, 0 arba 1

                node = (cmp2 < 0) ? leftRotation(node) : doubleLeftRotation(node); // išlyginam
            }

        } else if (node.getLeft() != null && node.getRight() != null) { //jeigu null, kaip ir medžio galas


            node.element = ((AVLNode<E>) getMax(node.getLeft())).element; //max kairės elementas?

            node.setLeft(removeRecursive(node.element, node.getLeft())); //šalinu kairį elemntą
        ...
        if (node != null){
            node.height = Math.max(height(node.getLeft()), height(node.getRight())) + 1; //didinu ugį
        }
