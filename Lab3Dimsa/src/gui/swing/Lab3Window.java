...
public class Lab3Window extends JFrame implements ActionListener {
    ...
    private MapADTx<String, Soldier> map;
    private int sizeOfInitialSubSet, sizeOfGenSet, colWidth, initialCapacity;
    private float loadFactor;
    private final SoldierMaker soldierMaker = new SoldierMaker();
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        try {
            ...
            if (source.equals(panButtons.getButtons().get(0))) {
                ...
                KsSwing.oun(taEvents, contaisSoldiers());
    ...
    public void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            Soldier[] autoArray = soldierMaker.makeAndSendSoldiers(sizeOfGenSet, sizeOfInitialSubSet);
            for (Soldier a : autoArray) {
                map.put(
                        soldierMaker.getSoldierIdFromBase(),//raktas
                        a);
            }
    ...
    public void mapAdd() throws MyException {
        Soldier a = soldierMaker.sendSoldier();
        map.put(
                soldierMaker.getSoldierIdFromBase(),// Raktas
                a);
        table.setModel(new TableModel(map.getModelList(panParam1.getTfOfTable().get(5).getText()),
    ...
    public String contaisSoldiers() {
        if (taInput.getText().equals("")) {
            return "Please, write ID";
        }
        if (map.contains(taInput.getText())) {
            return "The map contains soldier No " + taInput.getText();
        } else {
            return "The map does not contain soldier No " + taInput.getText();
        }
    }
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectedIndex()) {
            case 0:
                map = new MapKTUx<>(new String(), new Soldier(), initialCapacity, loadFactor, ht);
                break;
            case 2:
                map = new MapKTUOAx<>(new String(), new Soldier(), initialCapacity, loadFactor, ht);
                break;
