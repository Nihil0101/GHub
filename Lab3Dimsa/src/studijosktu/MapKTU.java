...
public class MapKTU<K, V> implements MapADTp<K, V> {
    ...
    /**
     * Papildo esamą MapKTU naujais elementais iš map
     * @param map
     */
    public void putAll(MapKTU<K, V> map){
        for (Node<K, V> node : map.table) {
            if (node != null) {
                V value = put(node.key, node.value);
            }
        }
    }

    /**
     * Pakeičia visas reikšmes, kurios lygios oldValue į newValue
     * @param oldValue
     * @param newValue
     */
    ...
    /**
     * Grąžina vidutinį grandinėlės ilgį
    ...
    /**
     * Suranda kiek yra laisvų vietų MapKTU
     * @return
     */
